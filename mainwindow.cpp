...
void MainWindow::createWidgetToplist()
{
    ...
    QString surl = "http://localhost:3000/top/playlist/highquality";
    QJsonDocument JD = QJsonDocument::fromJson(getReply(surl));
    qDebug() << surl;
    QJsonArray JA_list = JD.object().value("playlists").toArray();
    //qDebug() << list;
...
{
    qDebug() << "start to get data";
    QNetworkAccessManager *NAM = new QNetworkAccessManager;
    QNetworkRequest request;
    request.setUrl(QUrl(surl));
    request.setRawHeader("Referer", "http://localhost:3000/");
...
void MainWindow::createPlaylist(int id, QString name)
{
    QString surl = QString("http://localhost:3000/playlist/detail?id=%1").arg(id);
    qDebug() << surl;
    QJsonDocument JD = QJsonDocument::fromJson(getReply(surl));
    //qDebug() << JD;
    ...
    if (code == 200) {
        ...
        for (int i=0; i<tracks.size(); i++) {
            ...
            tableWidget_playlist->setItem(i,2,new QTableWidgetItem(tracks[i].toObject().value("al").toObject().value("name").toString()));
            ...
            tableWidget_playlist->setItem(i,5,new QTableWidgetItem(tracks[i].toObject().value("al").toObject().value("picUrl").toString()));
...
QString MainWindow::getMusicUrl(QString id)
{
    // QString id = tableWidget_playlist->item(row,4)->text();
    QString surl = "http://localhost:3000/song/url?id=" + id;
    qDebug() << surl;
    QJsonDocument JD = QJsonDocument::fromJson(getReply(surl));
    QJsonArray tracks = JD.object().value("data").toArray();
    QString url = tracks[0].toObject().value("url").toString();
    return url;
}

void MainWindow::downloadMusic(QString url)
{
    QNetworkRequest req;
    ...
    loop.exec();
    reply->deleteLater();
    QString path = QDir::currentPath() + "/hello.mp3";
    QFile out(path);
...
void MainWindow::playSong(int row, int column)
{
    ...
    QString id = tableWidget_playlist->item(row,4)->text();
    QString musicUrl = getMusicUrl(id);
//    QString surl = "http://localhost:3000/song/url?id=" + id;
//    qDebug() << surl;
    cout << "play";
    downloadMusic(musicUrl);
    player->setMedia(QUrl(QDir::currentPath() + "/hello.mp3"));
    player->setVolume(50);
...
void MainWindow::dialogDownload()
{
    ...
    lineEdit_url->setText(player->media().request().url().toString());
